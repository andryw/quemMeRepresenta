...
houseOfCunhaApp.filter('hasPresidenciavel', function() {
    return function(input,scope) {
        return _.filter(input, function (partido,index){
            return partido.nome in scope.presidente_por_partido;
        });
    }
});
...
houseOfCunhaApp.controller('VotacoesCtrl', ['$scope',  '$modal','$http', '$filter', function($scope,  $modal, $http,$filter) {
    ...
    $http.get('dados/coligacoes_candidaots.json').success(function(data) {

        $scope.presidenciaveis = data;
        $scope.presidente_por_partido = _.indexBy(data, 'partido');
    ...
    $scope.presidenciavel_pelo_partido =  function(partido) {
        return $scope.presidente_por_partido[partido].candidato
    };
