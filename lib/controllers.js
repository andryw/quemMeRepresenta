...
houseOfCunhaApp.controller('ModalDeputadoCtrl', function ($scope, $modalInstance, deputado, eleitor,indexesTemas) {
    ...
    $scope.getRoWColour = function(voto,index){
        ...
        if ((value >= 0) && indexVotoDeputado > -1){
            if (value == indexVotoDeputado){
                return "green";
            }else{
                return "red";
            }
...
houseOfCunhaApp.controller('VotacoesCtrl', ['$scope',  '$modal','$http', '$filter', function($scope,  $modal, $http,$filter) {
    ...
    $scope.filtroPartido =  function(item) {
        if($scope.partidoSelecionado == "" || $scope.partidoSelecionado == null){
            return true;
        }
        else {
            return $scope.partidoSelecionado.partido == item.partido;
        }
    };
